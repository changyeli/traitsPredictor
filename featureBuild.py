...
class featureBuild:
	def __init__(self):
		...
		self.labels = [] ## cluster labels for all data
	def readFile(self):
	...
	def fullKMeans(self):
		...
		km = KMeans(n_clusters = 5, algorithm = "auto", max_iter = 1000).fit(reduced)
		## store labels
		self.labels = km.labels_.tolist()
		...
		plt.title('K-means clustering on the full matrices (PCA-reduced data)\n'
		...
		plt.yticks(())
		plt.show()
	## write full data and labels into files
	def writeFile(self):
		## write allData to file
		self.allData.to_csv(self.root + "allData.csv", index = False, header = False)
		with open(self.root + "labels.txt", "w") as f:
